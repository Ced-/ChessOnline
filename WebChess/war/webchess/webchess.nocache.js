...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "webchess" because\n';
  ...
  $wnd.__gwt_activeModules['webchess'] = {
    'moduleName' : 'webchess',
    'bindings' : function() {
      return {};
    }
 ...
 * "/webchess.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['webchess'].superdevmode = true;

  var expectedSuffix = '/webchess.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + webchess + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:webchess';
  var devModeSessionKey = '__gwtDevModeSession:webchess';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/webchess/webchess.nocache.js';
  ...
  $wnd.__gwt_activeModules['webchess'].canRedirect = true;
  $wnd.__gwt_activeModules['webchess'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/webchess?user.agent=' + ua + '&_callback=' + callback;
